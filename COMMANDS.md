...
- `(L)` / `(M)` / `(U)` — пол **только в скобках**; каскад суффиксов (`for Woman` → `woman` → `for Her` → `Her` и зеркально для `(M)`) и «лишний токен = пол» — **только** при `(L)`/`(M)`. `Woman`/`Man` в названии без скобок (Ormonde Woman) — часть имени линии.
