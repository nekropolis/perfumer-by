...
import {
    fetchAdminDashboardWishlistProducts,
    type AdminDashboardViewedPeriod,
    type AdminDashboardWishlistProduct,
} from "@/lib/admin-dashboard-api";

const PERIOD_OPTIONS: { id: AdminDashboardViewedPeriod; label: string }[] = [
    { id: "year", label: "Год" },
...
export default function AdminWishlistProductsCard() {
    const [period, setPeriod] = useState<AdminDashboardViewedPeriod>("month");
    const [items, setItems] = useState<AdminDashboardWishlistProduct[]>([]);
    const [loading, setLoading] = useState(true);

    const loadItems = useCallback(async (currentPeriod: AdminDashboardViewedPeriod, signal?: AbortSignal) => {
        setLoading(true);
        try {
            const response = await fetchAdminDashboardWishlistProducts({ period: currentPeriod, signal });
            setItems(response.data.items);
        } catch (error) {
            if (error instanceof DOMException && error.name === "AbortError") {
                return;
            }
            console.error("Failed to load wishlisted products", error);
            setItems([]);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <AdminPageCard>
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                <div>
                    <div className="text-sm font-semibold text-admin-text">Топ избранного</div>
                    <div className="text-xs text-admin-text-secondary">
                        Топ-10 по добавлениям в избранное
                    </div>
                </div>
                ...
                <table className="min-w-full text-xs">
                    <thead className="bg-admin-muted/80 text-left text-[11px] font-semibold uppercase tracking-[0.06em] text-admin-text-secondary">
                        <tr>
                            ...
                            <th className="whitespace-nowrap px-2 py-1 text-right">Добавления</th>
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan={3} className="px-2 py-2 text-admin-text-secondary">
                                    Нет добавлений за выбранный период
                                </td>
                            </tr>
                        ) : (
                            items.map((item) => (
                                ...
                                >
                                    ...
                                    <td className="whitespace-nowrap px-2 py-1 text-right tabular-nums text-admin-text">
                                        {item.wishlists_count.toLocaleString("ru-RU")}
                                    </td>
