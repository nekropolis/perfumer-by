...
/**
 * Официальный графический знак белорусского рубля (НБРБ).
 * SVG с currentColor — совпадает по насыщенности с bold/semibold ценой.
 */
export default function BynSign({ className = "" }: BynSignProps) {
    return (
        <>
            <span className="sr-only">руб.</span>
            <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 361 447"
                aria-hidden
                focusable="false"
                className={`byn-sign ${className}`}
            >
                <path
                    fill="currentColor"
