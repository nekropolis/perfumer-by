...
export default async function ArticlesList({ limit = 6 }: { limit?: number }) {
    const items = await fetchCmsPosts({ type: "article", limit });

    return (
        <section className="space-y-4">
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold text-gray-900">Статьи</h2>
            </div>
                    ...
                    <article key={item.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white">
                        ...
                        <div className="space-y-2 p-4">
                            ...
                            <div className="flex items-center justify-between pt-1 text-sm">
                                ...
                                <Link href={`/articles/${item.id}`} className="font-medium text-black hover:underline">
                                    Перейти
                                </Link>
                            </div>
                        </div>
