        ...
        <AdminPageCard>
            <div className="space-y-4 rounded-2xl border bg-white p-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <button type="button" onClick={openCreate} className="rounded-xl border px-4 py-2 text-sm">
                        Добавить формулу
                    </button>
                </div>
                        ...
                        <div className="max-h-full w-full overflow-y-auto rounded-2xl bg-white p-5 shadow-xl">
                            ...
                            <div className="mt-4 grid gap-3 md:grid-cols-2">
                                ...
                                <label className="space-y-1 text-sm">
                                    ...
                                    <select className="w-full rounded-xl border px-3 py-2" value={form.source_type} onChange={(e) => setForm({ ...form, source_type: e.target.value as PriceFormulaPayload["source_type"], source_id: 0 })}>
                                ...
                                <label className="space-y-1 text-sm">
                                    <span>Источник</span>
                                    <select className="w-full rounded-xl border px-3 py-2" value={form.source_id || ""} onChange={(e) => setForm({ ...form, source_id: Number(e.target.value) })}>
                                ...
                                <label className="space-y-1 text-sm">
                                    ...
                                    <input type="number" step="0.01" className="w-full rounded-xl border px-3 py-2" value={form.multiplier} onChange={(e) => setForm({ ...form, multiplier: Number(e.target.value) })} />
                                </label>
                                <label className="space-y-1 text-sm">
                                    <span>Курс RUB</span>
                                    <input type="number" step="0.01" className="w-full rounded-xl border px-3 py-2" value={form.rub_rate} onChange={(e) => setForm({ ...form, rub_rate: Number(e.target.value) })} />
                                </label>
                                <label className="space-y-1 text-sm">
                                    <span>Коэффициент сложения</span>
                                    <input type="number" step="0.1" className="w-full rounded-xl border px-3 py-2" value={form.addend} onChange={(e) => setForm({ ...form, addend: Number(e.target.value) })} />
                                </label>
                                <label className="space-y-1 text-sm">
                                    <span>Округление до</span>
                                    <input type="number" min={0} max={4} className="w-full rounded-xl border px-3 py-2" value={form.round_precision} onChange={(e) => setForm({ ...form, round_precision: Number(e.target.value) })} />
                                </label>
                                <label className="space-y-1 text-sm md:col-span-2">
                                    <span>Режим правил варианта</span>
                                    <select className="w-full rounded-xl border px-3 py-2" value={form.variant_rule_mode} onChange={(e) => setForm({ ...form, variant_rule_mode: e.target.value as PriceFormulaPayload["variant_rule_mode"] })}>
                                ...
                                <label className="space-y-1 text-sm">
                                    ...
                                    <input type="number" className="w-full rounded-xl border px-3 py-2" value={form.sort_order} onChange={(e) => setForm({ ...form, sort_order: Number(e.target.value) })} />
                                </label>
                            ...
                            <div className="mt-4 flex justify-end gap-2">
                                <button type="button" className="rounded-xl border px-4 py-2 text-sm" onClick={() => setModalOpen(false)}>Отмена</button>
                                <button type="button" className="rounded-xl border bg-admin-primary px-4 py-2 text-sm text-white disabled:opacity-50" disabled={saving} onClick={() => void save()}>
