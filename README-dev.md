...
Файл `/etc/supervisor/conf.d/perfumer-queue.conf` (прод-пример: `scripts/supervisor/perfumer-queue.conf`):
...
`--timeout` воркера должен быть **не меньше** job `$timeout` (`RunPriceRefreshJob` /
`RunSellerOne*Job` = 7200 сек → воркер `7500`). `--max-time=0` (или ≥14400): иначе
после длинного job воркер выходит, supervisor `STARTING`, health-check шлёт ложный алерт.
Если `--timeout` воркера меньше job timeout, воркер убивает задачу, `lock->release()` в
middleware может не успеть, и ключ overlap остаётся в Redis до `expireAfter`.
